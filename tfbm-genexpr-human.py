...
export=pickle.load(open('/data/db/import/save/paralogs-export.pkl', 'rb'))

branch_order = ["Opisthokonta", "Bilateria", "Chordata", "Vertebrata", "Gnathostomata", "Euteleostomi", "Sarcopterygii", "Tetrapoda", "Amniota", "Mammalia", "Theria", "Eutheria", "Boreoeutheria", "Euarchontoglires", "Primates", "Haplorrhini", "Simiiformes", "Catarrhini", "Hominoidea", "Hominidae", "Homininae", "Homo sapiens"]
...
for x in sorted(exprdsnormed['dSbin'].unique()):
    if sum((exprdsnormed['dSbin'] == x)&(exprdsnormed['CpG-ness'] == 2)&(exprdsnormed['mut-pct'].notna()))>0 and sum((exprdsnormed['dSbin'] == x)&(exprdsnormed['CpG-ness'] == 0)&(exprdsnormed['mut-pct'].notna()))>0:
        ...
        print("Mann-Whitney test of TFBM (pct) on dSbin %d mixed vs. CGI-less (human), Expression and FDR controlled: %s"%(x, mannwhitneyu(exprdsnormed[(exprdsnormed['dSbin'] == x)&(exprdsnormed['CpG-ness'] == 1)]['mut-pct'], exprdsnormed[(exprdsnormed['dSbin'] == x)&(exprdsnormed['CpG-ness'] == 0)]['mut-pct'], alternative='greater').pvalue*len(exprdsnormed['dSbin'].unique())))
    else:
        print('Skipping %d'%x)
        dsSkip.add(x)
...
tfbm_scores = pickle.load(open('/data/db/import/save/human-tfbm-scores.pkl', 'rb'))
tfbm_scores_pct = pickle.load(open('/data/db/import/save/human-tfbm-scores-pct.pkl', 'rb'))
...
despine()
plt.savefig('human-tfbm-mutual-by-cgi-bp-with-baseline-expr-ctrl-mixed.pdf', dpi=600)
fig, ax = pyplot.subplots(figsize=(15,8))
pyplot.xticks(rotation=45)
...
g.legend(handles, ['CGI-less', 'mixed', 'CGI-full', 'baseline'])
despine()
plt.savefig('human-tfbm-mutpct-by-cgi-bp-with-baseline-expr-ctrl-mixed.pdf', dpi=600)
fig, ax = pyplot.subplots(figsize=(15,8))
...
g.legend(handles, ['CGI-less', 'mixed', 'CGI-full', 'baseline'])
despine()
plt.savefig('human-tfbm-mutual-by-cgi-ds-with-baseline-expr-ctrl-mixed.pdf', dpi=600)
fig, ax = pyplot.subplots(figsize=(15,8))
...
g.legend(handles, ['CGI-less', 'mixed', 'CGI-full', 'baseline'])
despine()
plt.savefig('human-tfbm-mutpct-by-cgi-ds-with-baseline-expr-ctrl-mixed.pdf', dpi=600)
