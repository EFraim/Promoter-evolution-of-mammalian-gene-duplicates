...
if __name__ == "__main__":
    ...
    exons={ d[0][5:]:d[2] for d in canonical_exons(db, r'/mnt/hddata/2/evgeny/TAU/Homo_sapiens.GRCh38.dna_sm.primary_assembly.fa')}
    coding_exons={ d[0][5:]:d[2] for d in canonical_coding_exons(db, r'/mnt/hddata/2/evgeny/TAU/Homo_sapiens.GRCh38.dna_sm.primary_assembly.fa')}
    cigs = {}
    from csv import reader
    for r in reader(open('/data/db/import/human_paralog_cigarlines.csv', 'r')):
        if not r[0] in cigs:
            cigs[r[0]] = ((r[7], r[3]),)
        else:
    ...
    export=pickle.load(open('/data/db/import/save/human-sim-local-export.pkl', 'rb'))
    ...
    export = export.assign(hasutr = [ 'five_prime_UTR' in (e.featuretype for e in db.children(db['gene:'+r['g1']])) and
                                      'three_prime_UTR' in (e.featuretype for e in db.children(db['gene:'+r['g1']])) and
                                      'five_prime_UTR' in (e.featuretype for e in db.children(db['gene:'+r['g2']])) and
                                      'three_prime_UTR' in (e.featuretype for e in db.children(db['gene:'+r['g2']]))
                                      for r in export.iloc ])
    pickle.dump(export, open('/data/db/import/save/paralogs-export-exocon.pkl', 'wb'))
    export[export['g1']<export['g2']].to_csv('human-paralogs-exocon.csv')
    export[(export['g1']<export['g2'])&export['hasutr']].to_csv('human-paralogs-exocon-utr.csv')
